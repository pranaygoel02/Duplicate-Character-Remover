...
function Home() {
  ...
  return (
    ...
    <div className='flex justify-center items-center flex-col gap-16 min-h-screen animate-up transition-all'>
        {/* <img src={Bg}/> */}
        <h1 className='capitalize font-bold text-3xl md:text-6xl text-center font-josefin'>Duplicate character remover</h1>
        <form onSubmit={handleSubmit} className='flex text-base sm:text-lg md:text-xl outline outline-1 outline-slate-300 rounded-md overflow-hidden font-poppins'>
            <input className='p-4 focus-within:outline-blue-500' value={string} onChange={e => setString(e.target.value)} placeholder='Enter string'/>
